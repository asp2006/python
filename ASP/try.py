...
def show_and_run_exercise(exercise_number):
    ...
    code_window.geometry("550x500")  # پنجره را کمی کوچک کرده‌ایم
    ...
    code_text = tk.Text(code_canvas, wrap="word", width=70, height=20)
    ...
    def run_code():
        try:
            ...
            if result == "":
                output_label.config(text="هیچ خروجی‌ای از کد برنخاست.")
            else:
                output_label.config(text=f"خروجی کد:\n{result}")
